...
from module.about_model import call_model, init_model
...
# about BM25
DATAPATH_2_INVERTED_INDEX_TABLE = \
    "/user_data/DG/hyde_with_table/BM25/Agri_inverted_agri_table_only_113-0309/table"
...
def hyde_table(model_and_tokenizer):
    """123
    """

    with open(DATAPATH_2_PROMPT, 'r', encoding='utf-8') as file:
        print("\t\tstart loading HEYDE_QUERY")
        query = json.load(file)["hyde_table"]
        print("\t\tend loading HEYDE_QUERY")

    with open(DATAPATH_2_RAW_JSON, 'r', encoding='utf-8') as file:
        print("\t\tstart loading loading")
        total_doc = json.load(file)
        print("\t\tend loading RAW_JSON")
